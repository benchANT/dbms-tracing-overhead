...
if args.path and not args.pids:
    if args.db == "mysql":
        ...
        regex = "pgstat_report_activity"
        symbols = BPF.get_user_functions_and_addresses(args.path, regex)
        if len(symbols) == 0:
            print("Can't find function 'pgstat_report_activity' in %s" % (args.path))
            exit(1)
        (postgresql_func_name, addr) = symbols[0]

        # For functions like triggers that create queries internally
        # regex1 = "CreateQueryDesc"
        # symbols1 = BPF.get_user_functions_and_addresses(args.path, regex1)
        # if len(symbols1) != 0:
        #     (postgresql_func_name1, addr1) = symbols1[0]
        mode = "POSTGRESQL"
...
struct temp_t {
    ...
    For queries from the MySQL shell only sizes up to 5039 are possible
...
BPF_PERCPU_ARRAY(data_buffer, struct data_t, 1);
BPF_PERCPU_ARRAY(temp_data_buffer, struct temp_t, 1);
...
Trace only packets with enum_server_command == COM_QUERY
    ...
    bpf_probe_read_user(&tmp->query, sizeof(tmp->query), (void*) PT_REGS_PARM2(ctx));
#else //USDT
...
/*
    ...
    struct temp_t *tempp;
    u64 pid = bpf_get_current_pid_tgid();
    tempp = temp_data_buffer.lookup(&zero);
    if (!tempp)
        return 0;
    ...
    if (delta >= THRESHOLD) {
        ...
        struct data_t* data = data_buffer.lookup(&zero);
        if (!data)
            return 0;  
        data->pid = pid >> 32;   // only process id
        data->timestamp = tempp->timestamp;
        data->duration = delta;
#if defined(MYSQL56) || defined(MYSQL57) || defined(POSTGRESQL)
...
""".replace("DEFINE_USDT", "#define USDT" if mode == "USDT" else "") \
    ...
    .replace("DEFINE_POSTGRESQL", "#define POSTGRESQL" if mode == "POSTGRESQL" else "") \
...
if mode.startswith("MYSQL"):
    ...
    # Uprobes mode
    bpf = BPF(text=program)
    bpf.attach_uprobe(name=args.path, sym=postgresql_func_name,
                      fn_name="query_start")
    bpf.attach_uretprobe(name=args.path, sym=postgresql_func_name,
                         fn_name="query_end")
    # For triggers
    # bpf.attach_uprobe(name=args.path, sym=postgresql_func_name1,
    #                   fn_name="query_start")
    # bpf.attach_uretprobe(name=args.path, sym=postgresql_func_name1,
    #                      fn_name="query_end")
...
if mode.startswith("MYSQL") or mode.startswith("POSTGRESQL"):
